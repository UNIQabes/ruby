...
static inline wideval_t WIDEVAL_WRAP(WIDEVALUE v)
{
    wideval_t w = {v};
    return w;
}
...
timegmw_noleapsecond(struct vtm *vtm)
{
     ...
     */
    ret = LONG2NUM(vtm->sec + vtm->min * 60 + vtm->hour * 3600);
    days_in400 = yday - 70 * 365 + DIV(year_mod400 - 69, 4) - DIV(year_mod400 - 1, 100) + (year_mod400 + 299) / 400;
...
#define TZMODE_SET_FIXOFF(time, tobj, off)                          \
    do                                                              \
    {                                                               \
...
utc_offset_arg(VALUE arg)
{
    ...
    if (!NIL_P(tmp = rb_check_string_type(arg)))
    {
        ...
        if (min)
        {
            if (!ISDIGIT(min[0]) || !ISDIGIT(min[1]))
                goto invalid_utc_offset;
            if (min[0] > '5')
                goto invalid_utc_offset;
...
{
    if (NIL_P(str = rb_check_string_type(str)))
        return Qnil;
...
#define expect_two_digits(x, bits) \
    (((unsigned int)(x = two_digits(ptr + 1, end, &ptr, #x)) > (1U << bits) - 1) ? rb_raise(rb_eArgError, #x " out of range") : (void)0)
        if (!peek('-'))
            break;
...
subsec_normalize(time_t *secp, long *subsecp, const long maxsubsec)
{
    ...
    else if (UNLIKELY(subsec < 0))
    {                                   /* subsec negative overflow */
...
rb_time_num_new(VALUE timev, VALUE off)
{
    ...
    if (!NIL_P(off))
    {
        ...
        {
            off = zone;
            if (NIL_P(zone = find_timezone(time, off)))
...
#define arg_range_check(v) \
    (((v) < 0) ? rb_raise(rb_eArgError, "%s must not be negative", tstr) : (void)0)
#else
#define arg_range_check(v) \
    ((interval && (v) < 0) ? rb_raise(rb_eArgError, "time interval must not be negative") : (void)0)
#endif
...
#define validate_vtm_range(mem, b, e) \
    ((vtm->mem < b || vtm->mem > e) ? rb_raise(rb_eArgError, #mem " out of range") : (void)0)
...
time_deconstruct_keys(VALUE time, VALUE keys)
{
    ...
    for (i = 0; i < RARRAY_LEN(keys); i++)
    {
        ...
        if (sym_month == key)
            rb_hash_aset(h, key, INT2FIX(tobj->vtm.mon));
        if (sym_day == key)
            rb_hash_aset(h, key, INT2FIX(tobj->vtm.mday));
        if (sym_yday == key)
            rb_hash_aset(h, key, INT2FIX(tobj->vtm.yday));
        if (sym_wday == key)
            rb_hash_aset(h, key, INT2FIX(tobj->vtm.wday));
        if (sym_hour == key)
            rb_hash_aset(h, key, INT2FIX(tobj->vtm.hour));
        if (sym_min == key)
            rb_hash_aset(h, key, INT2FIX(tobj->vtm.min));
        if (sym_sec == key)
            rb_hash_aset(h, key, INT2FIX(tobj->vtm.sec));
        if (sym_subsec == key)
        {
...
strftime_cstr(const char *fmt, size_t len, VALUE time, rb_encoding *enc)
{
    ...
    if (!str)
        rb_raise(rb_eArgError, "invalid format: %s", fmt);
    return str;
}
...
time_strftime(VALUE time, VALUE format)
{
    ...
    else
    {
        ...
        if (!str)
            rb_raise(rb_eArgError, "invalid format: %" PRIsVALUE, format);
        return str;
    }
}
